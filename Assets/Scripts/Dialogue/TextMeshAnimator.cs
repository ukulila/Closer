...
public class TextMeshAnimator : MonoBehaviour
{
    ...
    //Vertical
    public float wavevAmount = 1;
    public float wavevSpeed = 1;
    public float wavevSeparation = 1;
    public TextMeshAnimator_IndependencyType wavevIndependency = TextMeshAnimator_IndependencyType.Vertex;
    ...
    //Vertical
    private bool[] wavesvEnabled;
    private float[] wavevVelocities;
    private float[] wavevSpeeds;
    private float[] wavevSeparations;
    private TextMeshAnimator_IndependencyType[] wavevIndependencies;
    ...
    Vector3 wvv = new Vector3(); //Vertical
    ...
    string ParseText(string inputText)
    {
        ...
        //Vertical
        List<bool> wavesvEnabled = new List<bool>();
        List<float> wavevVelocities = new List<float>();
        List<float> wavevSpeeds = new List<float>();
        List<float> wavevSeparations = new List<float>();
        List<TextMeshAnimator_IndependencyType> wavevIndependencies = new List<TextMeshAnimator_IndependencyType>();
        bool wavingv = false;
        float wavevAmount = 1;
        float wavevSpeed = 1;
        float wavevSeparation = 1;
        TextMeshAnimator_IndependencyType wavevIndependency = this.wavevIndependency;
        ...
        for (int index = 0; index < inputText.Length; ++index)
        {
            if (inputText[index] == '<')
            {
                ...
                while (index < inputText.Length)
                {
                    if (inputText[index++] == '>')
                    {
                        ...
                        else if (tag.ToUpper().Contains("/VERTICAL"))
                        {
                            wavingv = false;
                            wavevAmount = 1;
                            wavevSpeed = 1;
                        ...
                        else if (tag.ToUpper().Contains("VERTICAL"))
                        {
                            ...
                            if (tag.ToUpper().Contains("UNITED"))
                            {
                                wavevIndependency = TextMeshAnimator_IndependencyType.United;
                            }
                            if (tag.ToUpper().Contains("WORD"))
                            {
                                wavevIndependency = TextMeshAnimator_IndependencyType.Word;
                            }
                            if (tag.ToUpper().Contains("CHARACTER"))
                            {
                                wavevIndependency = TextMeshAnimator_IndependencyType.Character;
                            }
                            if (tag.ToUpper().Contains("VERTEX"))
                            {
                                wavevIndependency = TextMeshAnimator_IndependencyType.Vertex;
                            }
                        }
            ...
            if (!char.IsControl(inputText[index]) && (inputText[index] != ' '))
            {
                ...
                wavesvEnabled.Add(wavingv);
                wavevVelocities.Add(wavevAmount);
                wavevSpeeds.Add(wavevSpeed);
                wavevSeparations.Add(wavevSeparation);
                wavevIndependencies.Add(wavevIndependency);
        ...
        this.wavesvEnabled = wavesvEnabled.ToArray();
        this.wavevVelocities = wavevVelocities.ToArray();
        this.wavevSpeeds = wavevSpeeds.ToArray();
        this.wavevSeparations = wavevSeparations.ToArray();
        this.wavevIndependencies = wavevIndependencies.ToArray();
    ...
    {
        return new Vector3(Mathf.Sin(time) * amount, Mathf.Sin(time) * wvv.x);
    }
    //Vertical
    Vector3 WavevVector(float amount, float time)
    {
        return new Vector3(Mathf.Sin(time) * wv.x, Mathf.Sin(time) * amount);
    }
    //WIGGLE
    ...
    void Update()
    {
        ...
        wvv = new Vector3(); //Vertical

        wgv = new Vector3(); //WIGGLE
        ...
        for (int i = 0; i < TMProGUI.textInfo.meshInfo.Length; ++i)
        {
            ...
            float wavevAmount = 1;
            if (wavevVelocities.Length > j)
            {
                wavevAmount = wavevVelocities[j];
            };

            float wavevSpeed = 1;
            if (wavevSpeeds.Length > j)
            {
                wavevSpeed = wavevSpeeds[j];
            };
            int wvl = 0;
            TextMeshAnimator_IndependencyType wavevIndependency = this.wavevIndependency;
            if (wavevIndependency == TextMeshAnimator_IndependencyType.United) wvv = WavevVector(this.wavevAmount, currentFrame * (this.wavevSpeed * wavevSpeed));
            ...
            for (int v = 0; v < TMProGUI.textInfo.meshInfo[i].vertices.Length; v += 4, ++j)
            {
                ...
                float wavevSeparation = this.wavevSeparation;
                if (wavevSeparations.Length > j)
                {
                    wavevSeparation = wavevSeparations[j];
                };

                TextMeshAnimator_IndependencyType prevWavevIndependency = wavevIndependency;
                if (j < wavevIndependencies.Length)
                {
                    wavevIndependency = wavevIndependencies[j];
                }
                if ((j >= 1) && (j < wavevIndependencies.Length + 1))
                {
                    prevWavevIndependency = wavevIndependencies[j - 1];
                }
                if (wavevIndependency == TextMeshAnimator_IndependencyType.Word)
                {
                    if (wl < TMProGUI.text.Length)
                    {
                        if ((TMProGUI.text[wvl] == ' ') || char.IsControl(TMProGUI.text[wvl]) || (prevWavevIndependency != TextMeshAnimator_IndependencyType.Word) || (wvl == 0))
                        {
                            wvv = WavevVector(this.wavevAmount, currentFrame * (this.wavevSpeed * wavevSpeed) + this.wavevSpeed * wavevSpeed + TMProGUI.textInfo.meshInfo[i].vertices[v].x / (this.wavevSeparation * wavevSeparation));
                            if ((TMProGUI.text[wvl] == ' ') || char.IsControl(TMProGUI.text[wvl])) ++wvl;
                        }
                    }
                }
                ++wvl;
                ...
                bool wavev = false;
                if (wavesvEnabled.Length > j)
                {
                    wavev = wavesvEnabled[j];
                }
                if (wavev)
                {
                    if (wavevVelocities.Length > j)
                    ...
                    if (wavevIndependency == TextMeshAnimator_IndependencyType.Character) wvv = WavevVector(this.wavevAmount, currentFrame * (this.wavevSpeed * wavevSpeed) + TMProGUI.textInfo.meshInfo[i].vertices[v].x / (this.wavevSeparation * wavevSeparation));
                    for (byte k = 0; k < 4; ++k)
                    {
                        if (wavevIndependency == TextMeshAnimator_IndependencyType.Vertex) wvv = WavevVector(this.wavevAmount, currentFrame * (this.wavevSpeed * wavevSpeed) + TMProGUI.textInfo.meshInfo[i].vertices[v + k].x / (this.wavevSeparation * wavevSeparation));
                        TMProGUI.textInfo.meshInfo[i].vertices[v + k] += wvv * wavevAmount;
                    }
                }
...
public class TextMeshAnimatorEditor : Editor
{
    ...
    SerializedProperty wavevAmount;
    SerializedProperty wavevSpeed;
    SerializedProperty wavevSeparation;
    SerializedProperty wavevIndependency;
    ...
    void OnEnable()
    {
        ...
        wavevAmount = serializedObject.FindProperty("wavevAmount");
        wavevSpeed = serializedObject.FindProperty("wavevSpeed");
        wavevSeparation = serializedObject.FindProperty("wavevSeparation");
        wavevIndependency = serializedObject.FindProperty("wavevIndependency");
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.LabelField("Vertical Properties", EditorStyles.boldLabel);
        EditorGUILayout.PropertyField(wavevAmount);
        EditorGUILayout.PropertyField(wavevSpeed);
        EditorGUILayout.PropertyField(wavevSeparation);
        EditorGUILayout.PropertyField(wavevIndependency);
